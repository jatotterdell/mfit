...
set.seed(7135)
dat1 <- simulate_continuous_outcome(
  nsubj = max(nseq), means = c(40, 30, 30, 30), sigma = 10, trunc = TRUE)
res1 <- simulate_trial_with_control3(nseq, dat = dat1, brar = 2, brar_k = k, brar_min = 0.15, dropeff = FALSE)
(plot_accrual(res1, 1) + labs(subtitle = "Interim 1") | plot_treat_means(res1, 1)) / 
(plot_accrual(res1, 2) + labs(subtitle = "Interim 2")| plot_treat_means(res1, 2))
```
...
```{r}
#| label: load-sim-data
sim_files <- grep("bounded_rar_[0-9]+", list.files(res_dir), value = T)
...
## Keep Effective Treatments

### $\epsilon = 0.98$

```{r}
#| label: tbl-oc-overall-098
...
```{r}
#| label: tbl-oc-summary-3-098
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR bounded.
...
```{r}
#| label: tbl-oc-overall-095
#| tbl-cap: Summary of trial operating characteristics across all arms, n = 10,000 simulations, no BRAR, $\epsilon = 0.95$.
oc_summary <- sim_final[eff_eps == 0.95 & trunc == FALSE,
  .(
    `Any effective` = any(p_eff > 0.95, na.rm = TRUE),
    `All effective` = all(p_eff > 0.95, na.rm = TRUE),
    `Any harmful` = any(i_inf == 1, na.rm = TRUE),
    `All harmful` = all(i_inf == 1, na.rm = TRUE)
  ),
  keyby = list(config, brar, brar_min, `True means`, trial)
...
```{r}
#| label: tbl-oc-summary-1-095
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, no BRAR, $\\epsilon=0.95$.
oc_summary <- sim_final[brar == 0 & eff_eps == 0.95 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
    Superior = sprintf("%.2f", mean(p_supr > 0.95)),
    Effective = sprintf("%.2f", mean(p_eff > 0.95)),
    Ineffective = sprintf("%.2f", mean(i_inf)),
    Active = sprintf("%.2f", mean(i_acti))
  ),
  keyby = list(config, Treatment = variable, Truth)
...
```{r}
#| label: tbl-oc-summary-2-095
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR unbounded.
oc_summary <- sim_final[brar == 2 & brar_min == 0 & eff_eps == 0.95 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
    Superior = sprintf("%.2f", mean(p_supr > 0.95)),
    Effective = sprintf("%.2f", mean(p_eff > 0.95)),
...
```{r}
#| label: tbl-oc-summary-3-095
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR bounded.
oc_summary <- sim_final[brar == 2 & brar_min == 0.15 & eff_eps == 0.95 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
    Superior = sprintf("%.2f", mean(p_supr > 0.95)),
    Effective = sprintf("%.2f", mean(p_eff > 0.95)),
...
## Drop Effective Treatments

```{r}
#| label: load-sim-data-dropeff
sim_files <- grep("bounded_rar2_[0-9]+", list.files(res_dir), value = T)
res <- lapply(sim_files, function(f) readRDS(paste0(res_dir, f)))
sim_config <- rbindlist(lapply(res, function(x) x[[1]]), idcol = "config")
sim_result <- rbindlist(lapply(res, function(x) x[[2]]), idcol = "config")
sim_runtim <- sapply(res, function(x) x[[3]]) # in seconds
...
```{r}
#| label: tbl-oc-overall-098-dropeff
#| tbl-cap: Summary of trial operating characteristics across all arms, n = 10,000 simulations, no BRAR, $\epsilon = 0.98$.
oc_summary <- sim_final[eff_eps == 0.98 & trunc == FALSE,
  .(
...
```{r}
#| label: tbl-oc-summary-1-098-dropeff
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, no BRAR.
oc_summary <- sim_final[brar == 0 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
...
```{r}
#| label: tbl-oc-summary-2-098-dropeff
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR unbounded.
oc_summary <- sim_final[brar == 2 & brar_min == 0 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
...
```{r}
#| label: tbl-oc-summary-3-098-dropeff
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR bounded.
oc_summary <- sim_final[brar == 2 & brar_min == 0.15 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
...
## Drop Effective Treatment if Superior

```{r}
#| label: load-sim-data-dropsup
sim_files <- grep("sim_final_supdrop_[0-9]+", list.files(res_dir), value = T)
res <- lapply(sim_files, function(f) readRDS(paste0(res_dir, f)))
sim_config <- rbindlist(lapply(res, function(x) x[[1]]), idcol = "config")
sim_result <- rbindlist(lapply(res, function(x) x[[2]]), idcol = "config")
sim_runtim <- sapply(res, function(x) x[[3]]) # in seconds
...
```{r}
#| label: tbl-oc-overall-098-dropsup
#| tbl-cap: Summary of trial operating characteristics across all arms, n = 10,000 simulations, $\epsilon = 0.98$.
oc_summary <- sim_final[eff_eps == 0.98 & trunc == FALSE,
  .(
    `Any as superior and effective` = any(i_suprandeff == 1, na.rm = TRUE),
    `Any superior at end` = any(p_supr > 0.98, na.rm = TRUE),
    `Any effective at end` = any(p_eff > 0.98, na.rm = TRUE),
    `All effective` = all(p_eff > 0.98, na.rm = TRUE),
    `Any harmful` = any(i_inf == 1, na.rm = TRUE),
    `All harmful` = all(i_inf == 1, na.rm = TRUE)
  ),
...
```{r}
#| label: tbl-oc-summary-1-098-dropsup
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, no BRAR.
oc_summary <- sim_final[brar == 2 & brar_min == 0 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
    Superior = sprintf("%.2f", mean(i_supr == 1)),
    `2nd best` = sprintf("%.2f", mean(i_2bestandeff == 1)),
    Effective = sprintf("%.2f", mean(i_eff == 1)),
    `Effective end` = sprintf("%.2f", mean(p_eff > 0.98)),
    Ineffective = sprintf("%.2f", mean(i_inf)),
    Active = sprintf("%.2f", mean(i_acti))
...
```{r}
#| label: tbl-oc-summary-2-098-dropsup
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR unbounded.
oc_summary <- sim_final[brar == 2 & brar_min == 0 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
...
```{r}
#| label: tbl-oc-summary-3-098-dropsup
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR bounded 0.1.
oc_summary <- sim_final[brar == 2 & brar_min == 0.10 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
...
```{r}
#| label: tbl-oc-summary-4-098-dropsup
#| tbl-cap: Summary of trial operating characteristics, n = 10,000 simulations, BRAR bounded 0.15.
oc_summary <- sim_final[brar == 2 & brar_min == 0.15 & eff_eps == 0.98 & trunc == FALSE,
  .(
    Allocated = sprintf("%.0f", mean(n_enr)),
