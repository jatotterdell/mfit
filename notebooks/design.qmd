...
matplot(
  ...
  lty = 1,
  xlab = expression(
    "Effect size, " ~ delta * ", where" ~ mu[1] - mu[0] ~ "=" ~ delta * sigma
  ),
  ylab = "Power"
)
segments(
  ...
  y1 = power[min(which(power[, 5] >= 0.8)), 5],
  lty = 2
)
segments(
...
plot_treat_means <- function(dat, i = 1) {
  ...
  ggplot(tmp, aes(Treatment, trt_mean)) +
    ...
    geom_text(
      aes(label = ifelse(is.na(p_eff), "", sprintf("%.2f", p_eff)), y = 56),
      size = 2.25
    ) +
    geom_text(
      aes(label = ifelse(is.na(p_supr), "", sprintf("%.2f", p_supr)), y = 60),
      size = 2.25
    ) +
    geom_text(aes(label = Decision, y = 52, colour = Decision), size = 2.25) +
    geom_text(
      aes(
        label = sprintf("%2.1f", trt_mean),
        x = Treatment,
        y = trt_mean + 4 * sqrt(trt_var)
      ),
      size = 2.25
    ) +
...
res1 <- simulate_trial_with_control3(
  nseq,
  dat = dat1,
  brar = 2,
  brar_k = k,
...
(plot_accrual(res1, 1) +
  labs(subtitle = "Interim 1") |
  plot_treat_means(res1, 1)) /
  (plot_accrual(res1, 2) +
    labs(subtitle = "Interim 2") |
    plot_treat_means(res1, 2)) /
...
res2 <- simulate_trial_with_control3(
  nseq,
  dat = dat2,
  brar = 2,
  brar_k = k,
...
(plot_accrual(res2, 1) +
  labs(subtitle = "Interim 1") |
  plot_treat_means(res2, 1)) /
  (plot_accrual(res2, 2) +
    labs(subtitle = "Interim 2") |
    plot_treat_means(res2, 2)) /
...
res3 <- simulate_trial_with_control3(
  nseq,
  dat = dat3,
  brar = 2,
  brar_k = k,
...
(plot_accrual(res3, 1) +
  labs(subtitle = "Interim 1") |
  plot_treat_means(res3, 1)) /
  (plot_accrual(res3, 2) +
    labs(subtitle = "Interim 2") |
    plot_treat_means(res3, 2)) /
...
#| fig-width: 6
set.seed(901358)
dat4 <- simulate_continuous_outcome(
  nsubj = max(nseq),
  means = c(35, 42, 44, 46),
  sigma = 10,
  trunc = TRUE
)
res4 <- simulate_trial_with_control3(
  nseq,
  dat = dat4,
  brar = 2,
  brar_k = k,
...
(plot_accrual(res4, 1) +
  labs(subtitle = "Interim 1") |
  plot_treat_means(res4, 1)) /
  (plot_accrual(res4, 2) +
    labs(subtitle = "Interim 2") |
    plot_treat_means(res4, 2)) /
...
#| fig-width: 6
set.seed(32413)
dat5 <- simulate_continuous_outcome(
  nsubj = max(nseq),
  means = c(40, 35, 33, 32),
  sigma = 10,
  trunc = TRUE
)
res5 <- simulate_trial_with_control3(
  nseq,
  dat = dat5,
  brar = 2,
  brar_k = k,
...
(plot_accrual(res5, 1) +
  labs(subtitle = "Interim 1") |
  plot_treat_means(res5, 1)) /
  (plot_accrual(res5, 2) +
    labs(subtitle = "Interim 2") |
    plot_treat_means(res5, 2)) /
...
sim_config[, `:=`(
  ...
  `True means` = sapply(means, function(x) {
    paste(sprintf("%.1f", x), collapse = ", ")
  })
)]
```
...
sim_final[,
  Truth := sprintf("%.1f", sapply(means, \(x) x[Treatment + 1])),
  by = 1:nrow(sim_final)
]
```
...
oc_summary <- sim_final[
  brar == 0 & eff_eps == 0.98 & trunc == FALSE,
  .(
...
oc_summary[,
  Treatment := factor(Treatment, labels = c("0 - control", "1", "2", "3"))
]
...
oc_summary <- sim_final[
  brar == 2 & brar_k == 0.25 & eff_eps == 0.98 & trunc == FALSE,
  .(
    `Any effective` = any(i_eff == 1, na.rm = TRUE),
    `All effective` = all(i_eff == 1, na.rm = TRUE),
    `Any harmful` = any(i_inf == 1, na.rm = TRUE),
...
ggplot(
  sim_final[
    brar == 2 & brar_k == 0.25 & trunc == FALSE,
    .(config, Effect, n_enr, variable)
  ],
  aes(variable, n_enr)
) +
...
oc_summary <- sim_final[
  brar == 2 & brar_k == 0.5 & eff_eps == 0.98 & trunc == FALSE,
  .(
    `Any effective` = any(i_eff == 1, na.rm = TRUE),
    `All effective` = all(i_eff == 1, na.rm = TRUE),
    `Any harmful` = any(i_inf == 1, na.rm = TRUE),
