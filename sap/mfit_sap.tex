...
\begin{itemize}\tightlist
  \item M-FIT Study Protocol version 1.1, 10 November 2021
...
The primary analysis will include all participants randomised to an intervention arm regardless of whether intercurrent events disrupted the allocated exercise regime and irrespective of adherence. That is, we will adopt an intention-to-treat (ITT) approach (de facto treatment-policy estimand) for the analyses with all randomised participants contributing to the analysis such that:

\begin{itemize}
  \tightlist
  \item patients are analysed in the group they were allocated to
  \item patients that do not receive or adhere (either partially or fully) to the intervention will be retained
...
All observed data for a participant will be included in the analysis, including any data collected after withdrawal from or non-adherence to the assigned intervention.

\subsubsection{Per-Protocol Analyses}

Secondary analyses will investigate per-protocol (de jure) effects if all participants had satisfactorily adhered to the interventions without withdrawal. These analyses will use the information collected in the post-exercise questionnaire on self-reported adherence on whether exercise was completed on that day.

A participant will be considered to have sufficiently adhered to the intervention if they complete at least x\% of exercise sessions.

Only follow-up data collected whilst a participant was determind to be sufficiently adherent to the intervention will be included in the analysis.
...
Denote by $j\in\{0,1,2,3, 4\}$ the visit occasion (0, 4, 8, 12, or 36 weeks), and by $k(i)\in\{0,1,2,3\}$ the treatment assigned to participant $i$. Denote by $y_{ij}$ the FACIT-Fatigue score for participant $i$ at visit $j$. Then, for $i=1,...,n$ and $j=0,1,2,3,4$,
$$
  \begin{aligned}
...
$$
  ...
  \end{aligned}
$$
and $x_{k(i)}$ is the $3\times 1$ intervention design vector (orthonormal) for participant $i$ who was assigned to intervention $k(i)$. Similarly, let $\mu_{kj}$ be the expected FACIT-Fatigue score at visit $j$ under treatment $k$.
...
The following provides brief specifications for the analyses of each of the secondary outcomes. For all secondary outcomes, the time-point of primary interest is considered to be 12 weeks after randomisation. However, due to the available repeated measures and potential for missing data, repeated measures models are specified to estimate the intervention differences at 12 weeks under MAR.

The secondary outcomes are divided into groups cosnsitent with their previous introduction in \ref{secondary-outcomes}:
...
This SAP reports only on the planned comparison and not on validation of the instrument.

The SONG-HD-F score will be analysed using a Bayesian ordinal mixed effects model. The model will include baseline score as part of the response, and have fixed terms for week of visit (linear),  the interaction between intervention and week of visit (forcing the baseline distribution to be shared across all treatment groups), random intercepts and slopes for participant, and random intercepts for site. Data from each available measurement occassion will be included for each participant.

The default model is
\begin{gather*}
  Y_{ij} \sim \text{Categorical}(\pi(\mu_{ij})) \\
  \text{logit}\left(\mathbb P(Y_{ij} \leq l)\right) = \kappa_{l} - \mu_{ij},\quad l=0,...,9 \\
  \pi_{l}(\mu) = \text{expit}(\kappa_l-\mu)-\text{expit}(\kappa_{l-1}-\mu)\\
  \mu_{ij} = \beta\texttt{week}_{ij} + (\texttt{week}_{ij}\times x_{k(i)})^\mathsf{T}\xi +
  \gamma_{i0}+\gamma_{i1}\texttt{week}_{ij} \\
  \pi(0)\sim \text{Dirichlet}(2/10,\cdots,2/10) \\
  \beta\sim \text{Normal}(0, 2.5),
  \quad \xi\sim \text{Normal}(0, 2.5),\quad
  \gamma_i\sim \text{Normal}(0, \Sigma) \\
  \Sigma= \text{diag}(\tau)\Omega{\mathsf{T}}\text{diag}(\tau),\quad
  \tau \sim \text{Student-t}^+(3, 0, 2.5),\quad
  \Omega \sim \text{LKJ}(1).
\end{gather*}

If issues are encountered, the model may be simplified by ignoring the site specific effects or the random slopes. If the assumption of linearity with respect to week of visit is inappropriate, then week may be transformed (e.g. sqrt(week)) or specified as a categorical term, or visit specific effects may be investigated independently. Similarly, if proportional odds assumptions is found inappropriate than non-proportional odds models may be investigated.
...
The aim is to use HADS to compare the effect of interventions on mood over time. Anxiety and depression scores will be analysed as separate but correlated responses. The sample cumulative logits will be explored. Each response will be analysed using ordinal regression (cumulative logistic with proportional odds) with fixed terms for week of visit, week of visit by intervention interaction (with no main effect for intervention forcing shared baseline across randomised groups), participant specific intercepts and slopes which are correlated across the outcomes, and site specific intercepts.

The default model is, for outcomes $m=1,2$ corresponding to the anxiety and depression score respectively
\begin{gather*}
  Y_{mij} \sim \text{Categorical}(\pi_m(\mu_{mij})) \\
  \text{logit}\left(\mathbb P(Y_{mij} \leq l)\right) = \kappa_{ml} - \mu_{ij} \\
  \pi_{ml}(\mu) = \text{expit}(\kappa_{ml}-\mu)-\text{expit}(\kappa_{m,l-1}-\mu) \\
  \mu_{mij} = \beta_m\texttt{week}_{ij} + (\texttt{week}_{ij}\times x_{k(i)})^\mathsf{T}\xi_m +
  \gamma_{mi0}+\gamma_{mi1}\texttt{week}_{ij} \\
  \pi_m(0)\sim \text{Dirichlet}(2/22,\cdots,2/22) \\
  \beta_m\sim \text{Normal}(0, 2.5),
  \quad \xi_m\sim \text{Normal}(0, 2.5),\quad
  \gamma_i\sim \text{Normal}(0, \Sigma) \\
  \Sigma= \text{diag}(\tau)\Omega\text{diag}(\tau),\quad
  \tau \sim \text{Student-t}^+(3, 0, 2.5),\quad
  \Omega \sim \text{LKJ}(1).
\end{gather*}

If issues are encountered, the model may be simplified by ignoring the site specific effects or the random slopes. If the assumption of linearity with respect to week of visit is inappropriate, then visit may be transformed (e.g. sqrt(week)) or specified as a categorical term, or visit specific effects may be investigated independently. Similarly, if proportional odds assumptions is found inappropriate than non-proportional odds models may be investigated.

\phantomsection
\label{analysis:promis-aspra}
\hyperref[outcome:promis-apsra]{\textbf{PROMIS-APSRA}}

The aim is to use the PROMIS Scale to compare the effects of each intervention on social participation over time. The aggregated score will be analysed using ordinal regression (cumulative logistic with proportional odds)  with fixed terms for week of visit, week of visit by intervention interaction (with no main effect for intervention forcing shared baseline across randomised groups), participant specific intercepts and slopes, and site specific intercepts.
...
The aim is to use the six-minute walk test (metres) to compare cardiorespiratory fitness at 12 and 36 weeks by intervention. Distance covered during the six-minute walk test will be analysed using log-normal mixed effect model for distance covered with fixed terms for treatment, visit, random intercept for participant and random intercept for site.
...
As a sensitivity analysis for the treatment policy estimand, multiple imputation will be done within groups defined by randomised intervention and intervention adherence at 12 weeks (assuming sufficient data in these groups). The imputation model will be as defind for the primary outcome, however, the model will be fit separately for participants deemed to have adhered to week 12 and those who are deemed to have not adhered to week 12.
...
\addsec{Appendix I: Trial Sample Size and Operating Characteristics}\label{trial-simulations}
